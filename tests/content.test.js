...
test('the hub is a valid degenerate chapter (no quest, one guide, doors to every real world)',()=>{const h=JSON.parse(fs.readFileSync(new URL('../data/hub.json',import.meta.url)));assert.equal(h.language,'en');assert.deepEqual(h.quest.items,[]);
 ...
 for(const key of ['doorEs','doorJa'])assert.ok(h.lines[key]?.who&&h.lines[key]?.option?.en,key);
 assert.ok(h.lines.guideRepeat?.en);
 for(const world of ['es','ja']){const c=load(world);assert.ok(h.lines[world==='es'?'doorEs':'doorJa'].who,`hub door line needs a speaker name for ${world}`)}
});
